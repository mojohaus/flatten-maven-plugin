 ...
 * The flattened POM is a reduced version of the original POM with the focus to contain only the important
 ...
 * <td>flattened POM contains the actual dependencies of the project. Test dependencies are removed. Variables
 * and {@link Model#getDependencyManagement() dependencyManagement} is resolved to get fixed dependency
 * attributes (especially {@link Dependency#getVersion() version}). If {@link #embedBuildProfileDependencies}
 * is set to <code>true</code>, then also build-time driven {@link Profile}s will be evaluated and may add
 * {@link Dependency dependencies}. For further details see {@link Profile}s below.</td>
 * </tr>
 * <tr>
...
public class FlattenMojo extends AbstractMojo {
  ...
  /**
   * The flag to indicate if the generated flattened POM shall be set as POM file to the current project. By
   ...
   * use <code>false</code> in order to only generate the flattened POM but never set it as POM file.
   */
  ...
  /**
   * Profiles activated by OS or JDK are valid ways to have different dependencies per environment. However,
   * profiles activated by property of file are less clear.
   * 
   * When setting this parameter to <code>true</code>, the latter dependencies will be written as direct
   * dependencies of the project. <strong>This is not how Maven2 and Maven3 handles dependencies</strong>.
   * When keeping this property <code>false</code>, all profiles will stay in the flattened-pom.
   */
   ...
   * @param headerComment is the content of a potential XML comment at the top of the XML (after XML
   *        declaration and before root tag). May be <code>null</code> if not present and to be omitted in
   *        target POM.
  ...
  /**
   * Writes the given <code>data</code> to the given <code>file</code> using the specified
   * <code>encoding</code>.
   * 
  ...
  protected Model createFlattenedPom(File pomFile) throws MojoExecutionException {
    ...
    ModelBuildingRequest buildingRequest = new DefaultModelBuildingRequest().setPomFile(pomFile).setModelResolver(
        resolver);
   ...
   * @return <code>true</code> if the given {@link Activation} is build-time driven, <code>false</code>
        ...
        if (isBuildTimeDriven(profile.getActivation())) {
          ...
          for (Dependency profileDependency : profileDependencies) {
            if (modelDependencies.contains(profileDependency)) {
              ...
              // Non build-time driven profiles will remain in the flattened POM with their dependencies and
   ...
   * 
   * @param effectiveModel is the effective POM {@link Model} to process.
   * @param flattenedDependencies is the {@link List} where to add the collected {@link Dependency
   *        dependencies}.
   */
   ...
   * @return the flattened {@link Dependency} or <code>null</code> if the given {@link Dependency} is NOT
   *         relevant for flattened POM.
   */
  ...
  /**
   * @return <code>true</code> if the generated flattened POM shall be
